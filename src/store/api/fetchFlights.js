...
export default function fetchFlights(args) {
  ...
  return secondDate ? (fetch(
    `https://avia-new.fstravel.com/api/wl-plugin/external/flightSearch/` + 
    `${fromTo}/${format(firstDate,'yyyy-MM-dd')}/${toFrom}/${secondDate ? format(secondDate,'yyyy-MM-dd') : "nextDate=false"}`+
    `/${passengers}/${args.flightClass}/false/z`
  )
  .then(res => res.json())
  .then(normalizer)
  .then(createFilter)) : 
  (fetch(
    `https://avia-new.fstravel.com/api/wl-plugin/external/flightSearch/` + 
    `${fromTo}/${format(firstDate,'yyyy-MM-dd')}/${passengers}/${args.flightClass}/false/z`
  )
  .then(res => res.json())
  .then(normalizer)
  .then(createFilter))
